...
class Pins:
    ...
    # 검사대 물체 감지는 IR이 아니라 카메라(YOLO)로 처리 — 별도 입력 핀 없음
...
CONVEYOR_SPEED = 0.4    # DC 모터 1 평상 듀티 사이클 (0.0 ~ 1.0)
# 출발 킥스타트: 정지마찰을 못 이겨 저속에서 안 도는 것 방지 — 출발 순간만 강하게 돌림
CONVEYOR_KICK_SPEED = 1.0   # 킥 듀티 (출발 시)
CONVEYOR_KICK_TIME  = 0.3   # 킥 지속 시간 (초). 0이면 킥 없음
