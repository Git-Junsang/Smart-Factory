...
2. 카메라(YOLO)에 대상 과일 감지 → 컨베이어(DC1) 정지·2초 안정화, LED2 ON, 다수결 추론
...
6. 불확실(신뢰도 부족) 시 → **서보 미동작**, 카운트 없이 다음으로(SKIP). 서보는 세 과일 중 하나로 확정됐을 때만 기울어짐
7. 매 사이클 후 카메라에서 과일이 사라질 때까지 대기 → 같은 과일 재감지 방지
8. 푸시버튼2 → OLED 카운트 전체 0으로 리셋
9. 푸시버튼1로 중단 시 어느 단계에서든 정지, LED3 ON

상태 LED는 항상 하나만 켜진다: **LED1=가동중 / LED2=검사·분류중 / LED3=중단중**.
검사대 물체 감지는 IR 센서가 아니라 **카메라(YOLO)**가 담당한다 — 별도 근접센서 불필요.
...
## 점검 스크립트
```bash
python -m scripts.test_hardware   # 컨베이어/분류레일/서보/LED/버튼 개별 점검
...
- **물체 감지는 카메라(YOLO)** — 별도 IR/PIR 근접센서를 쓰지 않는다. 카메라가 검사대(정지 지점)를 향하도록 설치.
